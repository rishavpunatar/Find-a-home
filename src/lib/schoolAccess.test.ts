...
import { schoolAccessPerPopulation, schoolAccessSubscore } from './schoolAccess'
...
describe('schoolAccess helpers', () => {
  ...
  it('builds a stronger subscore for better population-adjusted access', () => {
    const better = schoolAccessSubscore(140, 10_000)
    const worse = schoolAccessSubscore(50, 10_000)

    expect(better).toBeGreaterThan(worse)
  })

  it('still returns a deterministic score when population is missing', () => {
    const result = schoolAccessSubscore(18, null)
